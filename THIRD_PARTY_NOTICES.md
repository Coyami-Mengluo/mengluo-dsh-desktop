...
`package-lock.json` records exact Node package versions and integrity metadata. Keep third-party license files when repackaging. Build tools are not runtime imports: electron-builder uses MIT, and the SVG rasterizer `@resvg/resvg-js` uses MPL-2.0. Its generated placeholder PNG comes from the project's own [SVG source](assets/icon.svg), which is MIT-licensed. The private client's third-party illustration is excluded from this export.
