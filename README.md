...
Opening the plugin page uses cached data before dispatching a request. Search pages are cached for 5 minutes, browsing metadata for 15 minutes, and files addressed by an exact Git commit for up to 24 hours in the current client session. Identical in-flight GETs are shared; cancelling one caller does not cancel other consumers. The least-recently-used cache is bounded to 256 entries and 8 MiB of serialized JSON, and is cleared when the client exits. Manual refresh/check actions still revalidate mutable data; installation and update checks do not accept stale branches or npm versions when a fresh request fails. Repeated checks of the same available GitHub update reuse its immutable files instead of fetching them again.
...
Search is submitted only by clicking **Search** or pressing Enter. Typing, pausing, clearing the input and finishing IME composition do not send requests. Recent query pages are cached to reduce anonymous API requests. Submitting a newer query cancels or ignores stale responses so they cannot overwrite the newer results. A topic, listing or declared bundle is **not a security or compatibility certification**. Review the author, source, permissions, dependencies and license before installing; third-party plugins can execute code. Root packages that cannot be confirmed as installable Harness bundles require the author's manual installation instructions instead.
