...
(() => {
  ...
  const renderTiming = () => {
    ...
    timing.textContent = tr`已用 ${Math.floor(seconds / 60)}分${seconds % 60}秒 · 安装参考耗时 10–20 分钟，实际取决于网络和磁盘 · npm 安装上限 30 分钟`
  }
  const render = value => {
    current = value
    ...
    if (nextSourceKey !== sourceKey) {
      ...
      for (const item of value.downloadSources ?? []) {
        ...
        option.textContent = tr(item.label)
    ...
    sourceTest.textContent = value.connectionTesting ? tr('检测中…') : tr('检测连接')
    sourceDetail.textContent = tr(value.downloadActivity || value.downloadSources?.find(item => item.id === source.value)?.description || '')
    sourceStatus.hidden = !value.connectionTesting && !value.connectionResult
    sourceStatus.textContent = value.connectionTesting ? tr('正在检测当前下载源，请稍候…') : tr(value.connectionResult?.message ?? '')
    ...
    if (nextKey !== catalogKey) {
      ...
      for (const release of value.releases) {
        ...
        option.textContent = `${release.version} · ${release.preview ? tr('预览版') : tr('稳定版')}${release.recommended ? tr('（官方 latest）') : ''}`
    ...
    install.textContent = value.status === 'installing' ? tr('正在安装…') : value.status === 'starting' ? tr('正在启动…') : tr('安装并启动')
    status.textContent = tr(value.detail)
    reason.hidden = !value.reason
    reason.textContent = tr(value.reason)
    renderProgress(value.percent, { hidden: !busy })
    const activity = value.files
    const counters = []
    if (typeof value.percent === 'number') counters.push(tr`校验 ${value.percent}%`)
    if (activity?.completedFiles !== undefined) counters.push(tr`已处理 ${activity.completedFiles}${activity.totalFiles === undefined ? '' : ` / ${activity.totalFiles}`} 个文件`)
    if (activity?.registryRequests !== undefined) counters.push(tr`仓库请求 ${activity.registryRequests} 次`)
    if (activity?.resolvedDependencies !== undefined) counters.push(tr`已解析依赖 ${activity.resolvedDependencies} 个`)
    files.textContent = counters.join(' · ')
    renderTiming()
  }
  const unsubscribe = window.harnessSetup.onState(render)
  ...
  languageSelect.addEventListener('change', async () => {
    ...
    renderLanguage()
    try {
      if (!await setLanguagePreference(preference)) status.textContent = tr('语言设置未能保存，请重试。')
    } catch { status.textContent = tr('语言设置未能保存，请重试。') }
    finally { savingLanguage = false; renderLanguage() }
  })
  onLanguageChange(() => { catalogKey = ''; sourceKey = ''; if (current) render(current); renderLanguage() })
  renderLanguage()
  const report = error => { status.textContent = tr(error.message ?? String(error)) }
