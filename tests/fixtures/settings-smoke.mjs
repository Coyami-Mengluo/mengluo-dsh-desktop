...
async function run() {
  ...
  assert.deepEqual(actions.at(-1), { type: 'plugins-load' })
  ...
  const beforeTyping = actions.filter(action => action.type === 'plugins-search').length
  await contents.executeJavaScript("document.getElementById('plugin-search').focus(); document.getElementById('plugin-search').value = '極'; document.getElementById('plugin-search').dispatchEvent(new Event('input', {bubbles:true}))")
  await pause(650)
  assert.equal(actions.filter(action => action.type === 'plugins-search').length, beforeTyping, 'Pausing after a single character must not search')
  ...
  await pause(650)
  assert.equal(actions.filter(action => action.type === 'plugins-search').length, beforeTyping, 'Completing a keyword still requires explicit submission')
  await contents.executeJavaScript("document.getElementById('plugins-search-submit').click(); document.getElementById('plugins-search-submit').click()")
  await expect(() => actions.at(-1)?.type === 'plugins-search' && actions.at(-1).query === '極光')
  assert.equal(actions.filter(action => action.type === 'plugins-search').length, beforeTyping + 1, 'Double-clicking submits once')
  ...
  await contents.executeJavaScript("document.getElementById('plugins-search-submit').click()")
  ...
  await contents.executeJavaScript("document.getElementById('plugin-search').dispatchEvent(new CompositionEvent('compositionend', {bubbles:true}))")
  await pause(650)
  assert.equal(actions.filter(action => action.type === 'plugins-search').length, searchCount, 'Finishing IME composition does not search automatically')
  await contents.executeJavaScript("document.getElementById('plugin-search').dispatchEvent(new KeyboardEvent('keydown', {key:'Enter',keyCode:229,bubbles:true})); document.getElementById('plugin-search').dispatchEvent(new KeyboardEvent('keydown', {key:'Enter',repeat:true,bubbles:true}))")
  await pause(80)
  assert.equal(actions.filter(action => action.type === 'plugins-search').length, searchCount, 'IME confirmation and held Enter do not submit')
  ...
  await contents.executeJavaScript("document.getElementById('plugin-search').value = 'readme-only'; document.getElementById('plugin-search').dispatchEvent(new Event('input', {bubbles:true})); document.getElementById('plugins-search-submit').click()")
  ...
  const beforeClear = actions.filter(action => action.type === 'plugins-search').length
  await contents.executeJavaScript("document.getElementById('plugin-search').value = ''; document.getElementById('plugin-search').dispatchEvent(new Event('input', {bubbles:true}))")
  await pause(650)
  assert.equal(actions.filter(action => action.type === 'plugins-search').length, beforeClear, 'Clearing the input must not fetch the full catalog')
  await contents.executeJavaScript("document.getElementById('plugins-search-submit').click(); document.getElementById('plugin-tab-installed').click()")
  ...
  await pause(80)
  assert.equal(actions.filter(action => action.type === 'plugins-load').length, 1, 'Returning to the page does not re-fetch unnecessarily')
