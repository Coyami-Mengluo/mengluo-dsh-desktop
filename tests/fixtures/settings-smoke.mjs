...
async function run() {
  ...
  const state = {
    ...
    client: { currentVersion: '0.5.2', status: 'idle', supported: true, autoCheck: true },
    network: {
    ...
    plugins: {
      ...
      query: '', catalogLoading: false, loadingMore: false, catalogError: undefined,
    ...
    about: { productName: 'MengLuo DSH Desktop', clientVersion: '0.5.2', harnessVersion: '0.1.2-rc.1', logAvailable: true },
  }
  ...
  controller = createSettingsWindow({
    ...
    onAction: async request => {
      ...
      if (request.type === 'plugins-search') {
        ...
        Object.assign(state.plugins, { query: request.query, catalogLoading: true, loadingMore: false, catalogError: undefined, catalog: [], page: 0, total: 0, hasMore: false })
        ...
        Object.assign(state.plugins, { catalog, catalogLoading: false, page: 1, total: catalog.length, hasMore: false })
      }
  ...
  await expectDom(contents, "document.getElementById('plugins-catalog').hidden && !document.getElementById('plugins-catalog-status').textContent.includes('没有匹配')")
