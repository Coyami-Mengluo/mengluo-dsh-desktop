...
async function run() {
  ...
  await waitFor(() => settings.webContents.executeJavaScript("document.getElementById('panel-plugins').hidden === false && document.getElementById('plugins-runtime-notice').hidden === false && document.getElementById('plugins-catalog-error').hidden === false && document.getElementById('plugins-refresh').disabled === false"))
